...
static void queue_work_fn(struct work_struct* work_data)
{
    ...
    dev_ioctl(queue_work_data->fp, queue_work_data->cmd, queue_work_data->args);
...
static long dev_ioctl(struct file* fp, unsigned int cmd, unsigned long args)
{
	...
	if (cmd == MSG_QUEUE_LOAD)
    {
        ...
        size_t len = strnlen_user((const char __user*)args, PATH_MAX);
		char* path = kmalloc(len + 1, GFP_KERNEL);
		if (!path) return -ENOMEM;

		copy_from_user(path, (const char __user*)args, len + 1);
    ...
    if (cmd == MSG_QUEUE_SAVE)
    {
		...
		ret = queue_write(out_fp, file_write, old_queue.first);
    ...
    {
        struct queue_work_data_t* queue_work_data = kmalloc(sizeof(struct queue_work_data_t), GFP_KERNEL);
        if (!queue_work_data) return -ENOMEM;
        INIT_WORK(&queue_work_data->work, queue_work_fn);
        queue_work_data->fp = fp;
        queue_work_data->cmd = MSG_QUEUE_LOAD;
        queue_work_data->args = args;
